...
class DocumentRepository {
  ...
  Future<ErrorModel> unshareDocument(String token, String documentId) async {
    ErrorModel errorModel =
        ErrorModel(errorMessage: "Something went wrong", data: null);
    try {
      final res = await _client.delete(
        Uri.parse("$host/doc/unshare/$documentId"),
        headers: {
          "Content-Type": "application/json; charset=utf-8",
          "x-auth-token": token,
        },
      ...
      switch (res.statusCode) {
        case 200:
          errorModel = ErrorModel(
              errorMessage: null, data: "Document unshared successfully");
          break;
        case 404:
          errorModel =
              ErrorModel(errorMessage: "Document not found", data: null);
